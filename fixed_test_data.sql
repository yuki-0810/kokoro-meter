...
WITH weekly_test_data AS (
  SELECT 
    ...
    ARRAY[1, 2, 1, 0] as stage_levels,  -- 今週→1週前→2週前→3週前
    ARRAY[88, 92, 78, 85] as confidences,
    ARRAY[4, 7, 5, 6] as journal_counts
),
week_dates AS (
  SELECT 
    week_offset,
...
SELECT 
  '8c7d6043-beb4-477d-9e48-58d0e7708268'::uuid,
  week_start,
...
SELECT 
  ...
  AVG(stage_level::numeric) as avg_stage,
